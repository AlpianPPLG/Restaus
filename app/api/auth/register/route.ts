...
export async function POST(request: Request) {
  try {
    ...
    if (!username || !password || !full_name || !role) {
      return errorResponse("All fields are required", 400, corsHeaders);
    }

    const validRoles: UserRole[] = ["admin", "waiter", "kitchen", "cashier"];
    if (!validRoles.includes(role)) {
      return errorResponse("Invalid role specified", 400, corsHeaders);
    }
    ...
    if (existingUser) {
      return errorResponse("Username already taken", 409, corsHeaders);
    }
