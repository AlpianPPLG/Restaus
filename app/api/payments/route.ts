  ...
  try {
    ...
    if (!order_id || !payment_method || amount_paid === undefined) {
      return errorResponse(
        "Order ID, payment method, and amount paid are required",
        400,
        corsHeaders,
      );
    }
    ...
    if (!order) {
      return errorResponse("Order not found", 404, corsHeaders);
    }

    if (order.status === "completed") {
      return errorResponse("Order has already been paid", 400, corsHeaders);
    }

    if (order.status !== "delivered") {
      return errorResponse(
        "Order must be delivered before payment",
        400,
        corsHeaders,
      );
    }
    ...
    if (change_amount < 0) {
      return errorResponse("Insufficient payment amount", 400, corsHeaders);
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    return successResponse(payments, undefined, 200, corsHeaders);
  } catch (error) {
    return handleApiError(error, corsHeaders);
  }
}
